...
const LOBImbalanceChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const series = chart.addLineSeries({ color: '#00ff9c', lineWidth: 2 });
  ...
  useEffect(() => {
    ...
    seriesRef.current.update({ time, value: data.LOBImbalance });
  }, [data]);
...
export default memo(LOBImbalanceChart);
